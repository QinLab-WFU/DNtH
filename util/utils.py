...
# from models.RelaHash import RelaHash
try:
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    # state_dict = torch.load("./pretrained/vgg16.pth")  # 替换成你的模型权重文件路径
    #
    # # 重命名键并加载权重
    #
    # # 获取模型的当前状态字典
    ...
    # model_state_dict = models.state_dict()
    # new_state_dict = {}
    # models.load_state_dict(torch.load('./pretrained/vgg16.pth'))
    ...
    logger.info(f"Loaded weights successfully from '{config.MODEL.PRETRAINED}' for fine-tuning")
    ...
    #     else:
    #         torch.nn.init.constant_(models.head.bias, 0.)
    #         torch.nn.init.constant_(models.head.weight, 0.)
    #         del state_dict['head.weight']
    #         del state_dict['head.bias']
    #         logger.warning(f"Error in loading classifier head, re-init classifier head to 0")
    # # 修改 backbone.features
...
# def auto_resume_helper(output_dir='C:/Users/大禹/.cache/torch/hub/checkpoints'):  # 设置默认路径
#     checkpoints = os.listdir(output_dir)
#     checkpoints = [ckpt for ckpt in checkpoints if ckpt.endswith('pth')]
#     print(f"All checkpoints found in {output_dir}: {checkpoints}")
#     if len(checkpoints) > 0:
#         latest_checkpoint = max([os.path.join(output_dir, d) for d in checkpoints], key=os.path.getmtime)
#         print(f"The latest checkpoint found: {latest_checkpoint}")
#         resume_file = latest_checkpoint
#     else:
#         resume_file = None
#     return resume_file
...
# 使用默认路径调用函数
# latest_checkpoint = auto_resume_helper()
